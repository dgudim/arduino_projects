...
#define LED_PIN 11         // пин DI светодиодной ленты
#define MLED_PIN 13         // пин стандартого светодиода
#define MLED_ON HIGH         // пин стандартого светодиода
...
int hue;
unsigned long main_timer, hue_timer, color_timer, rainbow_timer, eeprom_timer;
float averK = 0.006;
byte count;
...
void setup() {
  ...
  Serial.print(F("FILLED_BRIGHT = ")); Serial.println(BRIGHTNESS);
  Serial.print(F("BRIGHT_SYNC = ")); Serial.println(brightnessSync);
#endif
}
...
void updateEEPROM() {
  ...
  EEPROM.updateInt(10, BRIGHTNESS);
  EEPROM.updateByte(11, brightnessSync);
}
void readEEPROM() {
  ...
  BRIGHTNESS = EEPROM.readInt(10);
  brightnessSync = EEPROM.readByte(11);
}
void eepromTick() {
  if (eeprom_flag)
