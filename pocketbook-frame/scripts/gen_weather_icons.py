#!/usr/bin/env python3
"""Rasterize Home Assistant weather SVGs into LVGL L8 images."""
...
.moon { fill: #000000; }
.rain { fill: #000000; }
.cloud-back { fill: #4a4a4a; }
.cloud-front { fill: #111111; }
.snow { fill: #ffffff; stroke: #000000; stroke-width: 0.6; paint-order: stroke; }
"""
...
PART_MOON = """
<path class="moon" d="m14.981 4.2112c0 1.9244-1.56 3.4844-3.484 3.4844-1.9244 0-3.4844-1.56-3.4844-3.4844s1.56-3.484 3.4844-3.484c1.924 0 3.484 1.5596 3.484 3.484"/>
"""
...
LIGHTNING = """
<path class="sun" d="m 9.9252695,10.935875 -1.6483986,2.341014 1.1170184,0.05929 -1.2169864,2.02141 3.0450261,-2.616159 H 9.8864918 L 10.97937,11.294651 10.700323,10.79794 h -0.508706 l -0.2663475,0.137936"/>
"""

EXCEPTIONAL = """
<circle cx="8.5" cy="8.5" r="6.4" fill="none" stroke="#000000" stroke-width="1.15"/>
<rect x="7.75" y="4.6" width="1.5" height="5.4" rx="0.4" fill="#000000"/>
<circle cx="8.5" cy="12.15" r="0.85" fill="#000000"/>
"""
...
RAIN_STATES = {"hail", "rainy", "pouring", "lightning_rainy"}
WINDY_STATES = {"windy", "windy_variant"}
SNOWY_STATES = {"snowy", "snowy_rainy"}
LIGHTNING_STATES = {"lightning", "lightning_rainy"}
...
def svg_for(name: str) -> str:
    ...
    if name == "sunny":
        ...
        parts.append(MOON)
    elif name == "partlycloudy_night":
        parts.append(PART_MOON)
    elif name == "partlycloudy":
        parts.append(PART_SUN)
    elif name == "exceptional":
        parts.append(EXCEPTIONAL)

    if name in CLOUDY_STATES:
        parts.append(CLOUDS)
    if name in RAIN_STATES:
        parts.append(RAIN)
    if name == "pouring":
        parts.append(POURING_EXTRA)
    if name in WINDY_STATES:
        parts.append(WIND)
    if name in SNOWY_STATES:
        parts.append(SNOW)
    if name in LIGHTNING_STATES:
...
def c_array(name: str, data: bytes) -> str:
    lines = [f"const LV_ATTRIBUTE_MEM_ALIGN uint8_t weather_icon_{name}_map[] = {{"]
    for offset in range(0, len(data), 16):
        chunk = ", ".join(f"0x{byte:02x}" for byte in data[offset : offset + 16])
        lines.append(f"    {chunk},")
    lines.append("};")
    lines.append("")
    lines.append(f"const lv_image_dsc_t weather_icon_{name} = {{")
    lines.append("    .header = {")
    ...
    lines.append(f"        .stride = {SIZE},")
    lines.append("        .reserved_2 = 0,")
    lines.append("    },")
    lines.append(f"    .data_size = {len(data)},")
    lines.append(f"    .data = weather_icon_{name}_map,")
...
def main() -> None:
    images = {name: rasterize(svg_for(name)) for name in ICONS}

    header = ROOT / "include" / "weather_icons.h"
    source = ROOT / "src" / "weather_icons.c"
    header.write_text(
        """#pragma once
...
"""
        + "\n".join(f"extern const lv_image_dsc_t weather_icon_{name};" for name in ICONS)
        + """
...
"""
        + "\n\n".join(c_array(name, images[name]) for name in ICONS)
        + "\n"
        + lookup
    )
